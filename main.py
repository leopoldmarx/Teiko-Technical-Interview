...
def show_home():
    ...
    st.write("Answered by Leopold Marx")
    st.write("Assigned May 31st, 2025")
...
def show_python():
    ...
    # st.write(st.session_state.cell_count_df)
...
selection, graph = st.columns([1,4])
...
with graph.expander('Detailed Statistics for 2b'):
    ...
    st.subheader('2b')
    st.write("""
After filtering treatment to "tr1", condition to "melanoma", and sample type to "PBMC", we have a total of 9 samples (6 response and 3 non-response) for 6 unique subjects (4 response and 2 non-response). It is generally recommended to have 30+ samples (ideally 30+ subjects) in each category before we can start making statistical infrances.

If we ignore the 30+ observations recommendation (for the purposes of a technical interview), the t-tests can be used to determine if there is a significant difference between the two groups. Under the "Detailed Statistics for 2b" expansion, we can see the results of running t-tests. The relative frequency for "cd4_t_cell" and "monocyte" populations are statistically different (t-test p-value < 0.05) between response and non-response groups. 
             
I came to this conclusion by first performing a Levene Variance test to determine if the two groups have significantly different variances. If the groups have significatly different variances, its recommended to perform the Welch's t-test over the standard t-test. Since all populations resulted in equal variances, I performed the standard t-test.
""")
...
def show_database():
    ...
    st.write("""Please see the ER Diagram below created in Lucid Chart. I think adding additional metrics like `date_remission` in `subject_condition` or more atrributes in the `subject` table like `weight`, `ethnicity`, and/or `height`. `treatment_date` in `subject_treatment` table would also be valuable. 
             
The `cell-count.csv` file provided had multiple samples from a given patient but only allowed one condition or treatment to be reported. I improved the format of the data by allowing the database to assign multipe treatments and multiple conditions to each subject. This allows for more complex analysis of treatment effectiveness if there is some interaction between mutliple treatments or conditions.""")
...
* Databases allow data to scale to incredible sizes, larger that what could fit on one computer alone.
* Creating databases usually implys there is an ETL process to load the database. Implementing ETL processes ensures more reliable data than manually updating databases.
* Data Integrity can be ensured with foreign keys, forced datatypes, and constrains.
...
def show_leopold():
    ...
    st.write("leopoldmarx@gmail.com")
    st.write("225-249-2842")
    st.write("https://www.linkedin.com/in/leopoldmarx/")
